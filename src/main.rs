...
use crate::customer::repo::postgres_repo::PostgresCustomerRepo;
...
async fn main() -> AppResult<()> {
	...
	let repo = PostgresCustomerRepo::new(database_connection);
	let repo: Arc<Box<dyn CustomerRepo + Send + Sync + 'static>> = Arc::new(Box::new(repo));

	let customer_service = CustomerServicesImpl::new(repo);
